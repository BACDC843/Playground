 ...
 *
 * Required env vars:
 *   GHL_API_KEY      – GoHighLevel private integration API key (CHS Media Group sub-account)
 *   GHL_LOCATION_ID  – GHL sub-account location ID (CHS Media Group)
...
function createMcpServer() {
  const server = new Server(
    { name: "ghl-chsmedia", version: "1.0.0" },
    { capabilities: { tools: {} } }
  );
...
app.get("/health", (_req, res) => res.json({ status: "ok", service: "ghl-chsmedia-mcp" }));
